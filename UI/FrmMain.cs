...
namespace Hix_CCD_Module
{
    public partial class FrmMain : Form
    {
        ...
        public FrmMain()
        {
            ...
            threadReadIni = new Thread(ReadIni);
            threadReadIni.IsBackground = true;
            //QueryMongo();
        }
            ...
            try
            {
                if(tsbtnRun.Text == "停止")
                {
                    if (IsExchange && isEnd == false)
                    {
                        ...
                        //btn = INIhelp1.GetValue(Path2D, "", "结束触发");
            ...
            try
            {
                ...
                if (DicTasks[CommonValue.TaskName].Outputs.Count > 0)
                {
                    foreach (Terminal t in DicTasks[CommonValue.TaskName].Outputs)
                    {
                        ...
                        if (t.ValueType == typeof(double) || t.ValueType == typeof(int) || t.ValueType == typeof(string) || t.ValueType == typeof(bool))
                        {
                            ...
                            if (CommonValue.ListRealValue.Count > indexResult)
                            {
                                ...
                                mFAIjudge.名称 = CommonValue.ListTitle[indexResult]; ;
                                mFAIjudge.是否判定 = true;
                    ...
                    if (!CommonValue.isCheck && CommonValue.isCheckToday)
                    {
                        ...
                        {
                            _iNGCount++;
                            this.BeginInvoke(new Action(() => resultViewing.ShowText(1, "NG", CogColorConstants.Red)));
                        }
                        else
                        {
                            this.BeginInvoke(new Action(() => resultViewing.ShowText(1, "OK", CogColorConstants.DarkGreen)));
                            _iOKCount++;
                        }
                ...
                if (DicTasks[CommonValue.TaskName].Outputs.Count > 0)
                {
                    ...
                    if (!CommonValue.isCheck && CommonValue.isCheckToday)
                    {
                        ...
                        {
                            _iNGCount++;
                            this.BeginInvoke(new Action(() => resultViewing.ShowText(2, "NG", CogColorConstants.Red)));
                        }
                        else
                        {
                            _iOKCount++;
                            this.BeginInvoke(new Action(() => resultViewing.ShowText(2, "OK", CogColorConstants.DarkGreen)));
                        }
        ...
        private void HikCamera_ImageTaked(object sender, HixDataTakedEventArgs e)
        {
            //isEnd = true;
            //自动运行状态
            if (CommonValue.IsStart && IsExchange)
            {
                bool imgGetOK = e.IsDone;
                if (imgGetOK)
                {
                    ...
                    if (e.CameraID == 0)
                    {
                        // Logging($"----! Index  [{indexOfImages}]", Color.Green);
                        //每次拍照时间 显示
                        Logging($"----! Index  [{indexOfImages}]" + "时间：" + DateTime.Now.ToString() + ":" + DateTime.Now.Millisecond, Color.Green);
                        if (!IsCache)
                        {
                            TempImages_Camera1.Add(image);
        ...
        private void toolChangeJob_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                //string[] _ArrWhite1 = INIhelp1.GetValue(PathOffet, "", "白色一穴").Split(',');
                //string[] _ArrWhite2 = INIhelp1.GetValue(PathOffet, "", "白色二穴").Split(',');
                ...
                //string[] _ArrPink1 = INIhelp1.GetValue(PathOffet, "", "粉色一穴").Split(',');
                //string[] _ArrPink2 = INIhelp1.GetValue(PathOffet, "", "粉色二穴").Split(',');


                //if ((_ArrWhite1.Length == _ArrWhite2.Length) && (_ArrSilver1.Length == _ArrSilver2.Length) && (_ArrBlack1.Length == _ArrBlack2.Length) && (_ArrPink1.Length == _ArrPink2.Length))
                //{
                ...
                //    {
                //        case 0:
                //            for (int i = 0; i < _ArrPink1.Length; i++)
                //            {

                //                DicTasks[CommonValue.TaskName].Outputs[i].AdjustCF = Convert.ToDouble(_ArrPink1[i]);
                //                DicTasks[CommonValue.TaskName].Outputs[i].AdjustCF2 = Convert.ToDouble(_ArrPink2[i]);
        ...
        private void toolChangeJob_Click(object sender, EventArgs e)
        {
